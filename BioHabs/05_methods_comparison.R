...
ssf_root <- file.path("results", "iSSA") # Force iSSA path
...
log_line(sprintf(
  "Starting 05_methods_comparison.R (mode=%s, run=%s, q=%.2f)",
  cfg$mode, toupper(opts$run), opts$q
), logf)
...
# Publication-ready panel plot function
plot_method_panel <- function(r_h2o, r_ssdm, r_ssf, sp, out_png) {
  # Helper to make df
  ...
  df_h2o <- make_df(r_h2o, "H2O (AutoML)")
  df_ssdm <- make_df(r_ssdm, "SSDM (Ensemble)")
  df_ssf <- make_df(r_ssf, "iSSA (Mechanistic)")

  df_all <- bind_rows(df_h2o, df_ssdm, df_ssf)
  ...
  # Create individual plots to combine (easier to control than faceting sometimes)
  plot_one <- function(dat, title) {
    ggplot(dat, aes(x = x, y = y, fill = val)) +
      geom_raster() +
      scale_fill_viridis_c(option = "viridis", limits = c(0, 1), oob = scales::squish, name = "Suitability", trans = "sqrt") +
      coord_equal() +
      labs(title = title) +
      theme_map
  }
  ...
  plots <- list()
  if (!is.null(df_h2o)) plots[[1]] <- plot_one(df_h2o, "H2O (AutoML)")
  if (!is.null(df_ssdm)) plots[[2]] <- plot_one(df_ssdm, "SSDM (Ensemble)")
  if (!is.null(df_ssf)) plots[[3]] <- plot_one(df_ssf, "iSSA (Mechanistic)")
  ...
  ggsave(out_png, p_combined, width = 6 * length(plots), height = 6, dpi = 300, bg = "white")
}

plot_raster_continuous <- function(r, title, out_png, center0 = FALSE, legend_title = "Suitability") {
  df <- as.data.frame(r, xy = TRUE, na.rm = TRUE)
  names(df) <- c("x", "y", "val")
  ...
  if (center0) {
    ...
    p <- p + scale_fill_gradientn(colors = c("#0c97f3", "#ffffff", "#e41b1e"), limits = c(-lim, lim))
  } else {
...
compute_pair_metrics <- function(rA, rB, q) {
  ...
  rA <- al$r1
  rB <- al$r2
  vA <- values(rA, mat = FALSE)
  vB <- values(rB, mat = FALSE)
  ...
  list(
    pearson = pearson, spearman = spearman, rmse = rmse, mae = mae, jaccard_hotspot_q = jac,
    rA = rA, rB = rB
  )
}
...
list_h2o_rep_preds <- function(sp) {
  ...
  if (!dir.exists(rep_dir)) {
    return(tibble(rep = integer(), path = character()))
  }
  ...
  rows <- lapply(reps, function(rn) {
    ...
    # Prefer standard naming: prediction_<SP>.tif
    cand <- file.path(rp, sprintf("prediction_%s.tif", sp))
    if (!file.exists(cand)) {
      # Fallback 1: generic pred.tif
      cand2 <- file.path(rp, "pred.tif")
      if (file.exists(cand2)) {
        message(sprintf("  [H2O %s/%s] Using fallback: pred.tif", sp, rn))
        cand <- cand2
      }
    }
    if (!file.exists(cand)) {
      # Fallback 2: any .tif (RISKY - may grab wrong file)
      tifs <- list.files(rp, pattern = "\\.tif$", full.names = TRUE)
      if (length(tifs)) {
        message(sprintf("  [H2O %s/%s] WARNING: Using first .tif found: %s", sp, rn, basename(tifs[1])))
        cand <- tifs[1]
      }
    }
...
list_ssdm_rep_preds <- function(sp) {
  ...
  rows <- lapply(reps, function(rn) {
    rp <- file.path(rep_dir, rn)
    # Standard naming: ESDM_<SP>_rep<N>.tif
    cand <- file.path(rp, sprintf("ESDM_%s_rep%d.tif", sp, as.integer(sub("^rep", "", rn))))
    if (!file.exists(cand)) {
      # Fallback: any .tif (RISKY - may grab wrong file)
      tifs <- list.files(rp, pattern = "\\.tif$", full.names = TRUE)
      if (length(tifs)) {
        message(sprintf("  [SSDM %s/%s] WARNING: Using first .tif found: %s", sp, rn, basename(tifs[1])))
        cand <- tifs[1]
      }
    }
    if (file.exists(cand)) tibble(rep = as.integer(sub("^rep", "", rn)), path = cand) else NULL
...
read_h2o_single <- function(sp) {
  ...
  # Standard naming: prediction_<SP>.tif
  ...
  if (length(tifs)) {
    message(sprintf("  [H2O %s single] WARNING: Using first .tif found: %s", sp, basename(tifs[1])))
    return(rast(tifs[1]))
  }
  NULL
}
...
read_ssdm_single <- function(sp) {
  ...
  tifs <- if (dir.exists(d)) list.files(d, pattern = "\\.tif$", full.names = TRUE) else character()
  if (length(tifs)) {
    message(sprintf("  [SSDM %s single] WARNING: Using first .tif found: %s", sp, basename(tifs[1])))
    return(rast(tifs[1]))
  }
  NULL
}
...
read_ssf_single <- function(sp) {
  ...
  # Standard naming: <SP>_SSF_rsf_0to1.tif (normalized to 0-1)
  cand <- file.path(d, sprintf("%s_SSF_rsf_0to1.tif", sp))
  if (file.exists(cand)) {
    return(rast(cand))
  }
  # Fallback 1: extension-less variant (legacy compatibility)
  cand2 <- file.path(d, sprintf("%s_SSF_rsf_0to1", sp))
  if (file.exists(cand2)) {
    message(sprintf("  [iSSA %s single] Using extension-less file", sp))
    return(rast(cand2))
  }
  # Fallback 2: any .tif (RISKY - may grab wrong file)
  tifs <- if (dir.exists(d)) list.files(d, pattern = "\\.tif$", full.names = TRUE) else character()
  if (length(tifs)) {
    message(sprintf("  [iSSA %s single] WARNING: Using first .tif found: %s", sp, basename(tifs[1])))
    return(rast(tifs[1]))
  }
  NULL
}
...
for (sp in species_all) {
  ...
  if (have_reps) {
    log_line(sprintf(
      "[replicates] %s | shared: %s",
      sp, paste(sort(reps_both), collapse = ", ")
    ), logf)
    ...
    plot_raster_continuous(
      rH_mean, paste0(sp, " — H2O mean suitability"),
      file.path(compare_root, "03_maps", sprintf("%s_H2O_mean.png", sp))
    )
    plot_raster_continuous(
      rS_mean, paste0(sp, " — SSDM mean suitability"),
      file.path(compare_root, "03_maps", sprintf("%s_SSDM_mean.png", sp))
    )

    if (length(diffs_HS)) {
      d_hs_mean <- do.call(c, diffs_HS) |> mean(na.rm = TRUE)
      writeRaster(d_hs_mean, file.path(
        compare_root, "01_between_methods/rasters",
      ...
      plot_raster_continuous(d_hs_mean, paste0("Δ (H2O − SSDM): ", sp),
        file.path(
          compare_root, "01_between_methods/plots",
    ...
    if (!is.null(r_ssf) && length(diffs_HF)) {
      ...
      plot_raster_continuous(
        rF_mean, paste0(sp, " — SSF suitability"),
      ...
      writeRaster(d_hf_mean, file.path(
        compare_root, "01_between_methods/rasters",
        sprintf("diff_mean_H2O_minus_SSF_%s.tif", sp)
      ),
      overwrite = TRUE, datatype = "FLT4S", gdal = c("COMPRESS=LZW", "BIGTIFF=IF_SAFER")
      )
      writeRaster(d_sf_mean, file.path(
        compare_root, "01_between_methods/rasters",
      ...
      # Generate 3-Method Panel (Replicates Mean)
      plot_method_panel(
        rH_mean, rS_mean, rF_mean, sp,
        file.path(compare_root, "03_maps", sprintf("%s_Method_Panel.png", sp))
      )
    }
  } else {
    ...
    log_line(sprintf(
      "[single] %s | H2O=%s, SSDM=%s, SSF=%s",
      sp, !is.null(rH), !is.null(rS), !is.null(rF)
    ), logf)

    # Generate 3-Method Panel (Single) if all exist
    if (!is.null(rH) && !is.null(rS) && !is.null(rF)) {
      plot_method_panel(
        rH, rS, rF, sp,
        file.path(compare_root, "03_maps", sprintf("%s_Method_Panel_Single.png", sp))
      )
    }
    ...
    if (!is.null(rH) && !is.null(rS)) {
      ...
      writeRaster(m$rA - m$rB, file.path(
        compare_root, "01_between_methods/rasters",
...
per_rep_df <- if (length(per_rep_rows)) {
  bind_rows(per_rep_rows)
} else {
  tibble(
    run = character(), dataset = character(), replicate = integer(),
...
per_single_df <- if (length(per_single_rows)) {
  bind_rows(per_single_rows)
} else {
  tibble(
    run = character(), dataset = character(), pair = character(),
