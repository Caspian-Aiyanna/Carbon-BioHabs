...
if (toupper(cfg$mode) == "REPRO") {
  ...
  log_line("Mode REPRO: Using ALL algorithms and 10 internal replicates.", logf)
} else {
  # FAST mode: Lightweight algorithms, fewer replicates
  algos <- algos_default
  cores <- 1L
  reps <- 2L # Minimal replicates for speed
  log_line("Mode FAST: Using lightweight algorithms and 2 internal replicates.", logf)
}

# Override from CLI if provided
if (!is.null(opts$algos)) {
  algos <- trimws(unlist(strsplit(opts$algos, ",")))
  log_line(sprintf("Overriding algorithms from CLI: %s", paste(algos, collapse = ", ")), logf)
}

# Validate algorithms
valid_algos <- c("GLM", "GBM", "RF", "MARS", "SVM", "ANN", "CTA")
algos <- intersect(algos, valid_algos)
if (!length(algos)) {
  log_line("No valid algorithms specified; falling back to lightweight defaults.", logf)
  algos <- algos_default
}

log_line(sprintf(
  "SSDM mode=%s, cores=%d, algorithms=%s, reps=%d",
  cfg$mode, cores, paste(algos, collapse = ","), reps
), logf)
...
for (f in sort(all_rep)) {
  ...
  # Prepare occurrence data for SSDM (needs lon, lat, species columns)
  occ_ssdm <- tmp[, c("lon", "lat")]
  occ_ssdm$species <- sp

  # Train SSDM ensemble model with LITERAL parameters
  log_line(sprintf(
    "  Training SSDM with %d algorithms, %d cores, %d reps",
    length(algos), cores, reps
  ), logf)
  ...
  esdm <- tryCatch(
    ...
    error = function(e) {
      log_line(sprintf(
        "  ERROR in SSDM training for %s (rep %d): %s",
  ...
  if (is.null(esdm)) {
    log_line(sprintf("Skip %s (rep %d) — SSDM training failed", sp, rep_idx), logf)
    next
  }

  # Save the SSDM model object
  out_rds <- file.path(out_dir, sprintf("ESDM_%s_rep%d.rds", sp, rep_idx))
  saveRDS(esdm, out_rds)
  # projection raster
  ...
  # Clean up memory after each replicate
  gc(verbose = FALSE)
}
